...
<div class=" mt-4">
	<div class=" flex justify-end">
        ...
        class=" flex w-26 justify-center gap-2 dark:bg-slate-700 bg-slate-700 text-gray-200 p-1.5 text-center {onCard?'rounded-t-md':'rounded-md' }">
	...
	{#if onCard}
        {#each pantOrders as order (order)}
            <ul class=" dark:bg-slate-700 bg-slate-700 p-2 px-4 rounded-l-lg  rounded-br-lg py-6">
                {#each Object.entries(order.measurements) as [key, value] (key)}
                    <li class=" flex justify-between py-1.5 border-0 border-b md:mx-4">
                        ...
                        {#if pantDirider[key]}
                            ...
                            <span class=" font-black text-gray-300">{Number(value) / pantDirider[key]}</span>
                        {:else}
