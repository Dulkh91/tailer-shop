...
<div class=" mt-4">
    ...
    class=" flex w-26 items-center gap-2  bg-white p-1.5 text-end dark:bg-slate-700 {onCard? 'rounded-t-md':'rounded-md'}">
		<Icon icon="ph:shirt-folded-fill" width="24" height="24" />
		កាត់អាវ
    </button>
	{#if onCard}
        {#each clothOrders as order (order)}
            <ul class=" p-2 px-4 py-6 shadow-lg  dark:bg-slate-700 rounded-b-lg rounded-tr-lg">
                {#each Object.entries(order.measurements) as [key, value] (key)}
                    <li class=" flex justify-between py-1.5 border-0 border-b md:mx-4">
                        ...
                        {#if clothDirider[key]}
                            ...
                            <span class=" font-black">{Number(value) / clothDirider[key]}</span>
                        {:else}
